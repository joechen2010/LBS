...
public class RichManUtilities {
	...
	private RichManManager richManManager = SpringContextHolder.getBean("richManManager");

	public static RichMan getRichManByPersonID(Long personID, EntityManager em) {
	...
	public static void adjustMoneyByCheckIn(Long placeID, Long personID) {
		EntityManager em = EMF.getInstance().createEntityManager();
		try {
	...
	public static boolean initRichMan(Long personID) {
		RichMan richMan = new RichMan(personID);
		RichManManager richManManager = SpringContextHolder.getBean("richManManager");
